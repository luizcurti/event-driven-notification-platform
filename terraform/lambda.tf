resource "aws_lambda_function" "notification_api" {
  ...
  depends_on = [aws_cloudwatch_log_group.lambda["notification-api"]]
...
resource "aws_lambda_function" "email" {
  ...
  depends_on = [aws_cloudwatch_log_group.lambda["email"]]
...
resource "aws_lambda_function" "sms" {
  ...
  depends_on = [aws_cloudwatch_log_group.lambda["sms"]]
...
resource "aws_lambda_function" "push" {
  ...
  depends_on = [aws_cloudwatch_log_group.lambda["push"]]
...
resource "aws_lambda_function" "retry_worker" {
  ...
  depends_on = [aws_cloudwatch_log_group.lambda["retry-worker"]]
...
# EventBridge invokes email/sms/push asynchronously; on invocation-level failure (distinct
# from the application-level failures already handled by the SQS retry queue/DLQ in sqs.tf),
# Lambda's own async retry (2 attempts) is exhausted and the event is sent here instead of
# being silently dropped.
locals {
  channel_lambda_function_names = {
...
resource "aws_lambda_function_event_invoke_config" "channel_async_failure" {
  for_each = local.channel_lambda_function_names

  function_name = each.value

  destination_config {
    on_failure {
      destination = aws_sqs_queue.retry_dlq.arn
    }
  }
}
