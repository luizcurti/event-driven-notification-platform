...
describe("localstack e2e notification flow", () => {
  ...
  it("lists notifications", async () => {
    ...
    const page = JSON.parse(response.body) as { items: Array<{ id: string }> };
    expect(page.items.some((item) => item.id === createdNotificationId)).toBe(true);
  });
  ...
  it("returns 400 when the JSON body is not an object", async () => {
    ...
    expect(response.statusCode).toBe(400);
    expect(JSON.parse(response.body)).toEqual({ message: "body must be a JSON object" });
  });
