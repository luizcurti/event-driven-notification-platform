...
describe("notification-api-lambda", () => {
  ...
  it("returns 200 for list notifications", async () => {
    listExecute.mockResolvedValue({ items: [{ id: "1", status: "DELIVERED" }] });
    const { handler } = await import("../../handlers/api/notification-api-lambda");
    ...
    expect(listExecute).toHaveBeenCalledWith({ limit: undefined, cursor: undefined });
  });

  it("paginates list notifications with limit and nextToken query params", async () => {
    ...
    const event = {
      ...
      body: null,
      pathParameters: null,
      queryStringParameters: { limit: "5", nextToken: "cursor-1" },
    } as unknown as APIGatewayProxyEvent;
    ...
    expect(listExecute).toHaveBeenCalledWith({ limit: 5, cursor: "cursor-1" });
  });
  ...
  it("returns 400 when body is a JSON array on POST", async () => {
    const { handler } = await import("../../handlers/api/notification-api-lambda");
    ...
    expect(result.statusCode).toBe(400);
    expect(JSON.parse(result.body)).toEqual({ message: "body must be a JSON object" });
  });
