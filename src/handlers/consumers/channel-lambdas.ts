...
import { NotificationRequestedDetail, processConsumerEvent } from "./process-consumer-event";
...
function createConsumerHandler(channel: Channel) {
  ...
  return async (event: EventBridgeEvent<string, NotificationRequestedDetail>): Promise<void> => {
    try {
