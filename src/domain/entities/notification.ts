...
// DynamoDB items are capped at 400KB; this leaves headroom for the rest of the item's attributes.
const MAX_PAYLOAD_SIZE_BYTES = 350_000;
...
export class Notification {
  ...
  static create(input: CreateNotificationInput): Notification {
    ...
    if (Buffer.byteLength(JSON.stringify(payload), "utf8") > MAX_PAYLOAD_SIZE_BYTES) {
      throw new DomainError(`payload exceeds maximum size of ${MAX_PAYLOAD_SIZE_BYTES} bytes`);
    }
